...
//   + Checks that each row (sub-initializer_list) has the same number of elements;
...
std::size_t compute_strides(const std::array<std::size_t, N> &exts, std::array<std::size_t, N> &strs) {
  std::size_t st = 1;
  for (int i = N - 1; i >= 0; --i) {
    strs[i] = st;
    st *= exts[i];
...
std::size_t compute_size(const std::array<size_t, N> &exts) {
  return std::accumulate(exts.begin(), exts.end(), 1, std::multiplies<std::size_t>{});
}
...
// Copies the elements of the tree of std::initializer_list s for a Matrix<T, N>.
template<typename T, typename Vec>
...
template<std::size_t N>
std::size_t do_slice(const MatrixSlice<N> &os, MatrixSlice<N> &ns) { return 0; }

template<std::size_t N, typename T, typename... Args>
std::size_t do_slice(const MatrixSlice<N> &os, MatrixSlice<N> &ns, const T &s, const Args &... args) {
